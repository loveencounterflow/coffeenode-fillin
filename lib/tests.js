...
(function() {
  ...
  this.test_data_lists = function() {
    var data, expected, result, template, templates_and_expectations, _i, _len, _ref, _results;
    templates_and_expectations = [['$name was captain on $0, $1, and $2', 'helo name']];
    ...
    for (_i = 0, _len = templates_and_expectations.length; _i < _len; _i++) {
      _ref = templates_and_expectations[_i], template = _ref[0], expected = _ref[1];
      result = FILLIN.fill_in(template, data);
      _results.push(log(TRM.green('test_data_lists'), TRM.grey(template), TRM.gold(result)));
    }
    return _results;
  };
  ...
  this.test_fill_in_container_3 = function() {
    var d;
    d = {
      foo: {
        bar: 'baz',
  ...
  this.main = function() {
    this.test_data_lists();
    this.test_recursive_expansions();
    return this.test_cycle_detection();
  };
