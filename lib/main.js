...
(function() {
  ...
  this.new_matcher = function(options) {
    var activator, closer, escaper, forbidden, opener, _ref, _ref1, _ref2, _ref3, _ref4;
    if (options == null) {
      options = {};
    }
  ...
  this.fill_in = function(template_or_container, matcher, data) {
    if (TYPES.isa_text(template_or_container)) {
      return this.fill_in_template(template_or_container, matcher, data);
    }
    return this.fill_in_container(template_or_container, matcher, data);
  };

  this.fill_in_template = function(template, matcher, data) {
    return this._fill_in_template.apply(this, [template].concat(__slice.call(this._get_matcher_and_data(matcher, data))));
  };

  this._fill_in_template = function(template, matcher, data) {
    var R, has_matched, result, seen, _ref;
    seen = {};
    R = template;
    while (true) {
      _ref = this._fill_in_template_once(R, matcher, data), has_matched = _ref[0], R = _ref[1];
      if (!has_matched) {
        return R;
      }
      if (seen[R] != null) {
  ...
  this._fill_in_template_once = function(template, matcher, data) {
  ...
  this.fill_in_container = function(container, matcher, data) {
    var change_count, error, errors, fill_in, m, _ref;
    _ref = this._get_matcher_and_data(matcher, data), matcher = _ref[0], data = _ref[1];
    if (data == null) {
      data = container;
    }
    errors = null;
    fill_in = (function(_this) {
      return function(matcher, sub_container, crumbs, old_value) {
        ...
        new_value = _this._fill_in_template(old_value, matcher, container);
        if (does_match) {
  ...
  this._get_matcher_and_data = function(matcher, data) {
    if (data != null) {
      return [matcher, data];
    }
    return [this.default_matcher, matcher];
  };
