...
(function() {
  ...
  this.get = function(container, locator_or_crumbs, fallback) {
    var R;
    R = (this.get_container_and_facet(container, locator_or_crumbs))[2];
    if (R === void 0) {
      if (fallback !== void 0) {
        return fallback;
      }
  ...
  this.get_container_and_facet = function(container, locator_or_crumbs) {
    if (TYPES.isa_list(locator_or_crumbs)) {
      return this.container_and_facet_from_crumbs(container, locator_or_crumbs);
    }
    return this.container_and_facet_from_locator(container, locator_or_crumbs);
  };
  ...
  this.new_method = function(matcher_hint) {
    var R, matcher;
    matcher = TYPES.isa_jsregex(matcher_hint) ? matcher : this.new_matcher(matcher_hint);
    R = (function(_this) {
  ...
  this._fill_in_template = function(template, data, matcher, serialize) {
    ...
    while (true) {
      ...
      _ref = this._fill_in_template_once(R, data, matcher, serialize), has_matched = _ref[0], R = _ref[1];
      if (!has_matched) {
  ...
  this._fill_in_template_once = function(template, data, matcher, serialize) {
    var R, container, head, is_full_length_name, key, markup, name, new_value, position, tail, _ref, _ref1;
    if (serialize == null) {
      serialize = true;
    }
    R = template;
  ...
  this.fill_in_container = function(container, data, matcher) {
    var change_count, error, errors, fill_in, m, serialize, _ref;
    ...
    fill_in = (function(_this) {
      return function(matcher, sub_container, crumbs, old_value) {
        ...
        new_value = _this._fill_in_template(old_value, data, matcher, serialize);
        if (does_match) {
